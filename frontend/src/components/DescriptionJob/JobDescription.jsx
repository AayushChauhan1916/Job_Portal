...
const JobDescription = () => {
  ...
  const handleApplyJob = async ()=>{
    try{
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/application/apply/${id}`,{
        method:"POST",
        credentials:"include"
      })
    ...
    const fetchJob = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/api/job/get/${id}`,
          {
            method: "POST",
            credentials: "include",
          }
        ...
        if (responseData.success) {
          ...
          const initiallyApplied = await fetchedJob?.application?.some(item=>item.applicant == user?._id);
  ...
  return (
    <div>
      <Navbar />
      {isLoading == true ? <JobDescriptionSkeleton/>:
      <div className="max-w-6xl mx-auto my-10 px-4">
        ...
        <div className="my-4">
          <h1 className="font-bold my-1">
            Role:{" "}
            <span className="pl-4 font-normal text-gray-800">{job?.position}</span>
          </h1>
          <h1 className="font-bold my-1">
            Location:{" "}
          ...
          <h1 className="font-bold my-1">
            Description:{" "}
            <span className="pl-4 font-normal text-gray-800">{job?.description}</span>
          </h1>
          <h1 className="font-bold my-1">
            Experience:{" "}
            <span className="pl-4 font-normal text-gray-800">{job?.experienceLevel
            } &nbsp;year</span>
          </h1>
          <h1 className="font-bold my-1">
            Salary:{" "}
            <span className="pl-4 font-normal text-gray-800"> {job?.salary}
            </span>
          </h1>
          <h1 className="font-bold my-1">
            Total Application:{" "}
            <span className="pl-4 font-normal text-gray-800">{job?.application?.length}</span>
          </h1>
          <h1 className="font-bold my-1">
            Posted Date:{" "}
            <span className="pl-4 font-normal text-gray-800">{job?.createdAt.split("T")[0]}</span>
          </h1>
        </div>
      </div>}
    </div>
